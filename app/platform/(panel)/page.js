...
export default async function PlatformHomePage() {
  ...
  return (
    <>
      <h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100 sm:text-2xl">Mis negocios</h1>
      ...
      {businesses.length === 0 ? (
        <div className="card mt-6 p-4">
          <EmptyState
            icon={Building2}
            description="Aún no tienes negocios. Crea el primero con el botón de arriba."
          />
        </div>
      ) : (
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          {businesses.map((b) => (
            <div key={b.id} className="card card-hoverable p-5">
              <p className="text-xs font-medium uppercase tracking-wide text-gray-400 dark:text-gray-500">
                {b.business_type_label || b.business_type}
              </p>
              <h2 className="mt-1 text-lg font-semibold text-gray-900 dark:text-gray-100">{b.name}</h2>
              <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                Admin: {b.admin_name} · {formatPhone(b.admin_phone)}
              </p>
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">/{b.slug}</p>
              <Link
                href={`/b/${b.slug}/admin`}
