...
import { INVITE_TTL_MINUTES, BOOKING_REJECTED_TITLE, BOOKING_REJECTED_BODY } from "./constants";
import {
  formatWeekRange,
...
export async function rejectAppointment(appointmentId, businessId) {
  const sql = getSql();
  const rows = await sql`
    UPDATE appointments
    SET status = 'cancelled', cancelled_by = 'rejected', updated_at = NOW()
...
export async function notifyCustomersRejected(businessId, appointments) {
  for (const apt of appointments) {
...
export async function getWeeklyReport(businessId, referenceDate = new Date()) {
  ...
  const openMinutes = parseBusinessMinutes(business.open_hour);
  const closeMinutes = parseBusinessMinutes(business.close_hour);
  ...
  const days = weekDates.map((date, index) => {
    ...
    for (const apt of appointments) {
      if (!["active", "completed"].includes(apt.status)) continue;
      const start = new Date(apt.start_at);
      if (start < dayStart || start > dayEnd) continue;
      bookedMinutes += (new Date(apt.end_at).getTime() - start.getTime()) / 60_000;
      appointmentCount += 1;
    }
  ...
  return {
    ...
    weekStart: toDateInputStr(weekDates[0]),
    weekEnd: toDateInputStr(weekDates[6]),
    spaceCount,
